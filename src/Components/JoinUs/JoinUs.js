import React from 'react';
import Header from '../Header/Header';
import { Button, Col, Form, Row } from 'react-bootstrap';
import './JoinUs.css';
import Footer from '../Footer/Footer';

const JoinUs = () => {
    return (
        <>
            <Header></Header>

            <h1 style={{ fontSize: '40px', margin: '10px', padding: '10px', color: 'green' }}>Join the Best Academy in town</h1>
            <Form className='form'>
                <Row className="mb-3">
                    <Form.Group as={Col} controlId="formGridEmail">
                        <Form.Label>Email</Form.Label>
                        <Form.Control type="email" placeholder="Enter email" />
                    </Form.Group>

                    <Form.Group as={Col} controlId="formGridPassword">
                        <Form.Label>Password</Form.Label>
                        <Form.Control type="password" placeholder="Password" />
                    </Form.Group>
                </Row>

                <Form.Group className="mb-3" controlId="formGridAddress1">
                    <Form.Label>Current Address</Form.Label>
                    <Form.Control placeholder="Your current Address" />
                </Form.Group>

                <Form.Group className="mb-3" controlId="formGridAddress2">
                    <Form.Label>Permenant Address</Form.Label>
                    <Form.Control placeholder="Your permanent Address" />
                </Form.Group>

                <Row className="mb-3">
                    <Form.Group as={Col} controlId="formGridCity">
                        <Form.Label>Your City</Form.Label>
                        <Form.Control />
                    </Form.Group>

                    <Form.Group as={Col} controlId="formGridState">
                        <Form.Label>Your State</Form.Label>
                        <Form.Select defaultValue="Choose...">
                            <option>Choose...</option>
                            <option>Kushtia</option>
                            <option>Dhaka</option>

                        </Form.Select>
                    </Form.Group>

                    <Form.Group as={Col} controlId="formGridZip">
                        <Form.Label>Zip Code</Form.Label>
                        <Form.Control />
                    </Form.Group>
                </Row>

                <Form.Group className="mb-3" id="formGridCheckbox">
                    <Form.Check type="checkbox" label="I accept all the Requerments" />
                </Form.Group>

                <Button variant="primary" type="submit">
                    Submit
                </Button>
            </Form>

            <Footer></Footer>

        </>
    );
};

export default JoinUs;